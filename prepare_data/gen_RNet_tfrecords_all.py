...
def _get_output_filename(output_dir, name, data_type, net):
    #st = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    #return '%s/%s_%s_%s.tfrecord' % (output_dir, name, net, st)
    #return '%s/train_PNet_landmark.tfrecord' % (output_dir)
    return '%s/%s_landmark.tfrecord' % (output_dir, data_type)


def run(dataset_dir, data_type, net, output_dir, name='MTCNN', shuffling=False):
    ...
    tf_filename = _get_output_filename(output_dir, name, data_type, net)
    print("tfrecord filename %s" % tf_filename)

    if tf.gfile.Exists(tf_filename):
        print('Dataset files already exist. Exiting without re-creating them.')
        return
    # GET Dataset, and shuffling.
    dataset = get_dataset(dataset_dir, data_type, net=net)
...
def get_dataset(dir, data_type, net='24'):
    #item = 'imglists/PNet/train_%s_raw.txt' % net
    #item = 'imglists/PNet/train_%s_landmark.txt' % net
    item = '%s/%s_%s.txt' % (net, data_type, net)
    if data_type == 'landmark':
        item = '%s/%s_%s_aug.txt' % (net, data_type, net)
    print item
...
if __name__ == '__main__':
    ...
    for data_type in ('pos', 'neg', 'part', 'landmark'):
        run(dir,data_type, net, output_directory, shuffling=True)
